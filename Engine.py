...
class Position:
    def __init__(self, board, psquare, wc, bc, history,
        ...
        # the move method.
        self.wc = wc  # [Queenside, Kingside] Castling rights for white
        self.bc = bc  # [Kingside, Queenside] Castling rights for black
            ...
            if piece in 'RQB' and checks[0] == 0:  # Sliders without checks
                for d in DirectionsDict[flipsquare][piece].keys():
                    for k in range(DirectionsDict[flipsquare][piece][d]):
                        ...
                        if square == 0 and self.wc[0] and q == 'K' and self.turn and abs(
                                k) == 3:  # Queen-side castling for white
                            ...
                            # move method easier)
                            break
                        elif square == 0 and self.wc[0] and q == 'K' and abs(
                                k) == 2 and not self.turn:  # King-side castling for black
                            if self.null_move(Move(j, j + W, '', 0)).rotate().checks_pins()[0][0] != 0 or \
                                    self.null_move(Move(j, j + W + W, '', 0)).rotate().checks_pins()[0][0] != 0:
                                break
                            yield Move(j, j + W + W, "", 1)  # castling is represented by the king move (makes
                            # move method easier)
                            break
                        if square == 7 and self.wc[1] and q == 'K' and self.turn and abs(
                                k) == 2:  # King-side castling for white
                            ...
                            break
                        elif square == 7 and self.wc[1] and q == 'K' and abs(
                                k) == 3 and not self.turn:  # Queen-side castling for black
                            if self.null_move(Move(j, j + E, '', 0)).rotate().checks_pins()[0][0] != 0 or \
                                    self.null_move(Move(j, j + E + E, '', 0)).rotate().checks_pins()[0][0] != 0:
                                break
                            yield Move(j, j + E + E, "", 1)
...
def alpha_beta(position, depth, alpha, beta, our_turn, hash_table, killer1, killer2, killer1_count, killer2_count,
                   iterative_moves, count, start_time, time_limit):
    if time.time()-start_time >= time_limit: # Time limit exceeded
        return None, None
    count += 1
    ...
    if our_turn:  # If we have to move, we want to maximize. This is ensured by the above
        ...
        for move in ordered_moves:
            ...
            alpha_beta(position, depth - 1, alpha, beta, False, hash_table, killer1, killer2,
                                        killer1_count, killer2_count, iterative_moves, count, start_time, time_limit)[0]
            killer1_count = 0
            if child_value == None: # If time limit is exceeded
                value1 = None
                break
...
def Search(position, time_limit):  # max time we want to consume in seconds
    ...
    iterative_moves = []
    iterative_break = False # To quit inside alpha_beta, when max time is exceeded
    iterative_list = []
    for depth in range(100):
        last_depth = (depth + 1)*2
        best_value, best_move = alpha_beta(position, last_depth, alpha, beta, True, {}, killer1,
                                            killer2, 0, 0, iterative_moves, 0, start_time, time_limit)
        iterative_list.append([best_value, best_move])
        if best_value == None:
            break
        end_time = time.time()
        elapsed_time = end_time - start_time
        iterarive_moves = [best_move]  # ¡¡¡NO METER SOLO EL PRIMER MOVIMIENTO!!!
    return ["Time taken:", elapsed_time, "seconds", "Best move: ", iterative_list[-2][1], "Evaluation: ", iterative_list[-2][0], 'Depth:',
            last_depth]
